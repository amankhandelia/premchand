...
class DesiDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        tokens = self.tokenizer(text, return_tensors='np', truncation=False)['input_ids'].squeeze()
        return tokens
...
def collate_fn(batch, tokenizer, seq_len: int, pmap: bool = False) -> Tuple[np.ndarray, np.ndarray]:
    ...
    if pmap:
        ...
        batch_x = rearrange(batch_x, "(device_count batch_size) seq_len->device_count batch_size seq_len", device_count=device_count)
        batch_y = rearrange(batch_y, "(device_count batch_size) seq_len->device_count batch_size seq_len", device_count=device_count)
